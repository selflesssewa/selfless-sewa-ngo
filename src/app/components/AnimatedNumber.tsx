...
type Props = { to: number; from?: number; as?: keyof JSX.IntrinsicElements; className?: string };
...
export const AnimatedNumber = ({ as, className, to, from = 0 }: Props) => {
  ...
  useEffect(() => {
    ...
    const controls = animate(from, to, {
      ...
      onUpdate(latest) {
        if (elRef.current) elRef.current.textContent = String(latest.toFixed()).padStart(String(to).length, "0");
      },
    });
